# Prompt-Engineer 提示词（设计/优化）完整报告
## 报告日期
2026-01-11

## 核心需求
增加功能：若要修改的文件是.agent.md或.prompt.md，则需要为他们添加合适的 YAML frontmatter。

## 目标文件上下文详情
- 文件路径：.github/agents/Prompt-Engineer.agent.md
- 文件用途：作为存放 Prompt-Engineer Agent 提示词的原目标文件；该提示词定义了如何基于目标文件上下文生成或优化提示词并输出双文件（原目标文件与设计报告）。
- 上下文核心：提示词需要严格遵守“结构化、可执行、可验证”原则，且在处理特殊文件类型（`.agent.md`、`.prompt.md`）时，应确保文件含有合适的 YAML frontmatter。输出格式需为 `.md`，且原目标文件仅包含纯提示词正文。
- 适配逻辑：在提示词中加入明确规则与执行流程步骤，检测目标文件扩展名并在写回或创建前验证/补齐 YAML frontmatter，参考 VS Code Copilot 自定义文档链接以保持兼容性。
